import {ArrowLeft, Database, FolderOpen} from '@phosphor-icons/react';
...
export function AdaptersScreen() {
	...
	const selected = adapters?.find(
		(adapter) => adapter.id === selectedAdapter
	);
	...
	const filteredAdapters = useMemo(() => {
		...
		return adapters.filter(
			(adapter) =>
				...
				adapter.data_type.toLowerCase().includes(q)
		);
		...
		<>
			...
			<div className="p-6">
				<div className="mb-4 flex flex-wrap items-center gap-2">
					<button
						onClick={() => navigate('/sources')}
						className="border-app-line bg-app-box text-ink hover:bg-app-hover inline-flex items-center gap-2 rounded-lg border px-3 py-1.5 text-sm font-medium transition-colors"
					>
						<Database size={14} weight="bold" />
						All Sources
					</button>
					<span className="text-ink-faint text-xs">
						Choose an adapter to create a new source, then return to
						Sources to review and sync it.
					</span>
				</div>
	...
	if (result) {
		return (
			<div className="flex flex-col items-center py-6">
				<p className="text-ink text-sm">Source added</p>
		...
		<div className="flex flex-col gap-3">
			...
			<div className="mt-1 flex gap-2">
				<button
					...
					disabled={createSource.isPending || syncSource.isPending}
				...
				>
					{createSource.isPending || syncSource.isPending
						? 'Adding...'
						: 'Add & Sync'}
				</button>
				<button
					onClick={onDone}
					disabled={createSource.isPending || syncSource.isPending}
					className="text-ink-faint hover:text-ink rounded-lg px-3.5 py-1.5 text-sm font-medium transition-colors disabled:opacity-40"
				>
					Cancel
				</button>
			</div>
